 ...
 * @return string
 */
function el_input_escape($str, $newlines = true) {
		$replacements = array(
				"\x00" => '\x00',
				"\n" => '\n',
				"'" => "\'",
 ...
 */
function el_emojis_to_entites($content) {
...
function input($input, $noencode = '', $default = false, $strip = true) {
		...
		$hook = el_call_hook('el', 'input', false, array(
				'input' => $input,
				'noencode' => $noencode,
				'default' => $default,
				'strip' => $strip,
		...
		if ($hook) {
				...
				if (strlen($str) > 0) {
						$str = el_emojis_to_entites($str);
						if ($strip) {
								return trim(el_input_escape($str));
						} else {
								return el_input_escape($str);
						}
				}
		}
		return false;
 ...
 * @return string
 */
function el_restore_new_lines($string, $br = false) {
		if (strlen($string) == 0) {
				return false;
		}
		$replacements = array(
 ...
 * @return void
 */
function el_set_input($name, $value) {
