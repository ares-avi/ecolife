 ...
 * @license   Open Source Social Network License (el LICENSE)  https://www.opensource-socialnetwork.org/licence
 ...
 */
function el_image_resize_parameters($width, $height, $options) {
 ...
 */
function el_resize_image($input_name, $maxwidth, $maxheight, $square = FALSE, $x1 = 0, $y1 = 0, $x2 = 0, $y2 = 0, $upscale = FALSE) {
    ...
    $params = el_image_resize_parameters($width, $height, $options);
    if ($params == false) {
        return false;
    }
	//elFile to support animated gif photos #1473
	if ($load_function == 'imagecreatefromgif' && el_is_hook('el','image:resize:gif')) {
			$image_resize_options = array(
					'max_width' => $maxwidth,
					'max_height' => $maxheight,
					'file_path' => $input_name,
			);
			return el_call_hook('el', 'image:resize:gif', $image_resize_options, false);
	}
    // load original image
    ...
    //quality set 
    $imagejpeg_quality = el_call_hook('el', 'image:resize:quality', false, 90);
 ...
 */
function el_save_image($file, $destination) {
    $info = getimagesize($file);
    $image_quality = el_call_hook('el', 'image:save:quality', false, 90);
