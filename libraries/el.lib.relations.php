 ...
 * @return boolean
 */
function el_add_relation($from, $to, $type) {
		if(!empty($from) && !empty($to) && !empty($type) && $type !== 0) {
				$add              = new ElDatabase;
				$params['into']   = 'el_relationships';
				$params['names']  = array(
						'relation_from',
						'relation_to',
						'type',
				...
				if($add->insert($params)) {
						el_trigger_callback('relation', 'add', array(
								'from' => $from,
								'to' => $to,
								'type' => $type
						));
 ...
 * @return objects
 */
function el_get_relationships(array $params = array()) {
		$wheres   = array();
		$vars     = array();
		$database = new ElDatabase;
		if(isset($params['from']) && !empty($params['from'])) {
				if(isset($params['inverse'])) {
						$vars['joins'] = array(
								'JOIN el_relationships as r1 ON r1.relation_from=r.relation_to'
						);
		...
		if(isset($params['to']) && !empty($params['to'])) {
				if(isset($params['inverse'])) {
						$vars['joins'] = array(
								'JOIN el_relationships as r1 ON r1.relation_to=r.relation_from'
						);
		...
		$vars['from']   = 'el_relationships as r';
		$vars['wheres'] = array(
 ...
 * @return boolean
 */
function el_delete_relationship_by_id($id) {
		if(!empty($id)) {
 ...
 * @return boolean
 */
function el_update_relationship_by_id($id, $vars = array()) {
 ...
 * @return boolean
 */
function el_delete_relationship(array $vars = array()) {
 ...
 *
 * @param  ElUser $user Entity of user 
 *
 * @return bool
 */
function el_delete_user_relations($user) {
		if($user) {
				$delete           = new ElDatabase;
				$params['from']   = 'el_relationships';
 ...
 * @return bool
 */
function el_delete_group_relations($group) {
		if($group) {
				$delete           = new ElDatabase;
				$params['from']   = 'el_relationships';
