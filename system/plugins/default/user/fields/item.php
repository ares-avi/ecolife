		...
		foreach($params['items'] as $fields) {
				if(isset($fields['text'])) {
						foreach($fields['text'] as $item) {
								...
								$args['placeholder'] = el_print("{$item['name']}");
								if(isset($item['class'])){
								...
								if(isset($item['label']) && !is_bool($item['label'])){
									...
									echo "<label>".el_print("{$item['name']}")."</label>";
								}
								echo el_plugin_view('input/text', $vars);
								echo "</div>";
						}
				}
				if(isset($fields['textarea'])) {
						foreach($fields['textarea'] as $item) {
								...
								echo el_plugin_view('input/textarea', $vars);
								echo "</div>";
						}
				}
				if(isset($fields['dropdown'])) {
						foreach($fields['dropdown'] as $item) {
								...
								echo el_plugin_view('input/dropdown', $args);
								echo "</div>";
						}
				}						
				if(isset($fields['radio'])) {
						foreach($fields['radio'] as $item) {
								...
								echo el_plugin_view('input/radio', $args);
								echo "</div>";
						}
				}		
		}
