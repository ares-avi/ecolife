 ...
 * @return bool
 */
el.ajaxRequest = function($datap){
    $(function(){
		
        var $form_name = $datap['form'];
        $('body').on("submit", $form_name, function(event){
													
			var $data = el.call_hook('ajax', 'request:settings', null, $datap);	
        	var url = $data['url'];
 ...
 * @return bool
 */
el.PostRequest = function($datap){
	var $data = el.call_hook('ajax:post', 'request:settings', null, $datap);
	var url = $data['url'];
	var callback = $data['callback'];
	var error = $data['error'];
