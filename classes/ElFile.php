...
class ElFile extends ElEntities {
		/**
		 * DeleteDir
		...
		public function allowedFileExtensions() {
				...
				return el_call_hook('file', 'allowed:extensions', null, $types);
		}
		/**
		 * getFileUploadError
		...
		public function getFileUploadError($code) {
				...
				return el_print("upload:file:error:$key");
		}
		/**
		 * showFileUploadError
		...
		public function showFileUploadError() {
				...
				if(isset($this->file) && ($this->file['error'] !== UPLOAD_ERR_OK || $this->file['size'] == 0)) {
						el_trigger_message($this->getFileUploadError($this->file['error']), 'error');
		...
		public function addFile() {
				if(isset($this->file) && !empty($this->file) && !empty($this->owner_guid) && !empty($this->subtype) && !empty($this->type)) {
						...
						if($this->typeAllowed()) {
								...
								$this->dir = el_get_userdata("{$this->type}/{$this->owner_guid}/{$this->path}");
								if(!is_dir($this->dir)) {
								...
								if($fileguid = $this->add()) {
										...
										if(preg_match('/image/i', $this->file['type'])) {
												...
												$image_res = el_call_hook('file', 'image:resolution', $this, $image_res);
												
												//compress image before save
												$filecontents = el_resize_image($this->file['tmp_name'], $image_res['width'], $image_res['height']);
										}
		...
		public static function mimeTypes() {
				...
				return el_call_hook('file', 'mimetypes', false, $mimetypes);
		}
		/**
		 * Get file
		 * 
		...
		public function getPath() {
				if(isset($this->guid)) {
						$path = el_get_userdata("{$this->type}/{$this->owner_guid}/{$this->value}");
						return $path;
				}
				return false;
		}
