...
class ElEntities extends ElDatabase {
		/**
		...
		private function initAttributes() {
				...
				$this->types = el_call_hook('entities', 'types', false, array(
						'object' => 'ElObject',
						'user' => 'ElUser',
						'annotation' => 'ElAnnotation',
						'entity' => 'ElEntities',
						'site' => 'ElSite',
						'component' => 'ElComponents'
				));
				...
				if(!isset($this->page_limit)) {
						...
						$this->page_limit = el_call_hook('pagination', 'per_page', false, 10);
				}
				if(!isset($this->count)) {
						$this->count = false;
				}
		 ...
		 *           		$this->entity_permission => EL_ACCESS
		...
		public function add() {
				...
				if(!empty($this->owner_guid) && in_array($this->type, $this->entity_types)) {
						...
						if($this->insert($this->params)) {
								...
								$this->params['into']   = 'el_entities_metadata';
								$this->params['names']  = array(
										'guid',
										'value'
								);
								...
								el_trigger_callback('entity', 'created', $args);
		...
		public function get_entity() {
				...
				$params['joins']  = "JOIN el_entities_metadata as emd ON e.guid=emd.guid";
				$params['wheres'] = array(
						...
						if($entities) {
								foreach($entities as $entity) {
										if(isset($this->datavars[$entity->subtype])) {
												$params['table']  = 'el_entities_metadata';
												$params['names']  = array(
														'value'
												);
												$params['values'] = array(
		...
		public function deleteEntity($guid = '') {
				...
				$vars	= array();
				$vars['entity'] = $guid;
				el_trigger_callback('entity', 'before:delete', $vars);
				
				if($this->delete($params)) {
						$metadata['from']   = 'el_entities_metadata';
						...
						$vars['entity'] = $guid;
						el_trigger_callback('delete', 'entity', $vars);
						return true;
				}
				return false;
		}
		...
		public function searchEntities(array $params = array()) {
				...
				$default = array(
						...
						'order_by' => false,
						'offset' => 1,
						'page_limit' => el_call_hook('pagination', 'per_page', false, 10), //call hook for page limit
						'count' => false
				);
		...
		public function canChange($user = '') {
				...
				if(isset($user->guid) && $user instanceof ElUser) {
						if((isset($this->owner_guid) && $this->type == 'user' && $this->owner_guid == $user->guid) || el_isAdminLoggedin()) {
								$allowed = true;
						}
				}
