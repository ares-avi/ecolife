...
class ElAnnotation extends ElEntities {
		/**
		 * 
		 *
		 * @return void
		...
		public function addAnnotation() {
				...
				$create = el_call_hook('annotation', 'create', array(
						'params' => $params,
						'instance' => $this
				), true);
				if($create) {
						if($this->ElDatabase->insert($params)) {
								$this->annotation_inserted = $this->ElDatabase->getLastEntry();
								...
								el_trigger_callback('annotations', 'created', $params);
		...
		public function deleteAnnotation($annotation = 0) {
				...
				el_trigger_callback('annotation', 'before:delete', $vars);				
				
				if($this->deleteByOwnerGuid($annotation, 'annotation')) {
						$this->statement("DELETE FROM el_annotations WHERE(id='{$annotation}')");
						if($this->execute()) {
								$data = el_get_userdata("annotation/{$annotation}/");
								if(is_dir($data)) {
										ElFile::DeleteDir($data);

										//rmdir($data);
								}
								$params['annotation'] = $annotation;
								el_trigger_callback('annotation', 'delete', $params);
								return true;
						}
				}
				return false;
		...
		public function searchAnnotation(array $params = array()) {
				...
				if(isset($options['entities_pairs']) && is_array($options['entities_pairs'])) {
						foreach($options['entities_pairs'] as $key => $pair) {
								...
								if(!empty($pair['name']) && isset($pair['value']) && !empty($operand)) {
										...
										$params['joins'][] = "INNER JOIN el_entities as e{$key} ON e{$key}.owner_guid=a.id";
										$params['joins'][] = "INNER JOIN el_entities_metadata as emd{$key} ON e{$key}.guid=emd{$key}.guid";
								}
						}
				...
				$params['joins'][] = "INNER JOIN el_entities as e ON e.owner_guid=a.id";
				$params['joins'][] = "INNER JOIN el_entities_metadata as emd ON e.guid=emd.guid";
				...
				//prepare search	
				$params['from']     = 'el_annotations as a';
				$params['params']   = array(
						"{$distinct}a.id",
						'a.time_created',
						'a.owner_guid',
		...
		public function canChange($user = '') {
				...
				$allowed = false;
				if(isset($user->guid) && $user instanceof ElUser) {
						if((isset($this->owner_guid) && $this->owner_guid == $user->guid) || el_isAdminLoggedin()) {
								$allowed = true;
						}
				}
				return el_call_hook('user', 'can:change', $this, $allowed);
		}
		/**
		...
		public function save() {
				if(isset($this->id) && !empty($this->id)) {
						$params['table']  = 'el_annotations as a';
						$params['names']  = array(
								'owner_guid',
								'subject_guid'
						);
