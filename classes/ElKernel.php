...
class ElKernel extends ElSystem {
		/**
		...
		private static function storagePath(){
				...
				return el_get_userdata($kernel_storage.'/');			
		}
		/**
		 * Clear Storage
		 * 
		...
		static function clearStorage(){
				$kernel_storage = hash('md5', 'kernel_storage');
				$kernel_storage = "ks_".$kernel_storage;
				$storage 		= el_get_userdata($kernel_storage.'/');			
				ElFile::DeleteDir($storage);
		}		
		/**
		 * Set cache data
		 *
		...
		public static function getCred() {
				if(function_exists('el_kernal_creds')) {
						return el_kernal_creds();
				}
				return false;
		}
		/**
		...
		public function sendRequest($method, $params) {
				if(!empty($method) && isset($params)) {
						...
						foreach($params as $item){
							$vars['website_url'] = el_site_url();
							$vars['api_key']     = $creds->api_key;
							$vars['secret']      = $creds->secret;
							$vars['website']     = el_site_url(); //website_url is different param then website.
							$vars['email']       = el_site_settings('owner_email');
							$vars['admin']       = $emails_list;
							$vars['notifcation'] = el_site_settings('notification_email');
							$vars['site_name']   = el_site_settings('site_name');
		...
		private function handShake($endpoint, array $options = array()) {
				...
				foreach($options as $key => $option){
					...
					curl_setopt($mrcurl[$key]['curl'], CURLOPT_CAINFO, el_route()->www . 'vendors/cacert.pem');
		...
		public static function setINIT() {
				global $El;
				$handle           = new ElKernel;
				if(isset($El->kernelBatch)){
					foreach($El->kernelBatch as $key => $item){
						$data[$key]['pci']      = $item[0];
