...
class ElObject extends ElEntities {
		/**
		...
		public function __construct($guid = 0) {
				...
				if(!empty($guid)) {
						$object = el_get_object($guid);
						if($object) {
								foreach($object as $item => $val) {
										$this->{$item} = $val;
								}
						} else {
								//el DB exceptions must start with 1<exception code>
		...
		public function addObject() {
				...
				$create           = el_call_hook('object', 'create', array(
						'params' => $params,
						'instance' => $this
				), true);
				if($create) {
						if($this->insert($params)) {
								...
								el_trigger_callback('object', 'created', $args);
		...
		public function deleteObject($object = '') {
				...
				$vars	= array();
				$vars['guid'] = $object;
				el_trigger_callback('object', 'before:delete', $vars);
				//delete entites of (this) object
				if($this->deleteByOwnerGuid($object, 'object')) {
						$data = el_get_userdata("object/{$object}/");
						if(is_dir($data)) {
								ElFile::DeleteDir($data);
						}
				}
				$delete['from']   = 'el_object';
				$delete['wheres'] = array(
						"guid='{$object}'"
				);
				if($this->delete($delete)){
						el_trigger_callback('object', 'deleted', $vars);
						return true;
				}
				return false;
		}
				...
				if(isset($options['entities_pairs']) && is_array($options['entities_pairs'])) {
						foreach($options['entities_pairs'] as $key => $pair) {
								...
								if(!empty($pair['name']) && isset($pair['value']) && !empty($operand)) {
										...
										$params['joins'][] = "JOIN el_entities as e{$key} ON e{$key}.owner_guid=o.guid";
