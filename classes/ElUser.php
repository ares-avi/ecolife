 ...
 *
 */
class ElUser extends ElEntities {
		/**
		...
		public function initAttributes() {
				...
				$this->ElAnnotation = new ElAnnotation;
				$this->notify         = new ElMail;
		...
		public function addUser() {
				...
				if(empty($user->username) && $this->isPassword() && $this->isUsername()) {
						...
						$password_encryption_alog = el_call_hook('user', 'password:algorithm', false, 'bcrypt');
						...
						$this->sendactiviation = el_call_hook('user', 'send:activation', false, $this->sendactiviation);
						$this->validated       = el_call_hook('user', 'create:validated', false, $this->validated);
						if($this->validated === true) {
								//don't set null , set empty value for users created by admin
								$activation = '';
						}
						$params['into']   = 'el_users';
						$params['names']  = array(
								'first_name',
								'last_name',
								'email',
						...
						$create           = el_call_hook('user', 'create', array(
								'params' => $params,
								'instance' => $this
						), true);
						if($create) {
								if($this->insert($params)) {
										...
										if(!empty($guid) && is_int($guid)) {
												...
												//add user entities 
												$extra_fields = el_call_hook('user', 'signup:fields', $this, $fields);
										...
										if($this->sendactiviation === true) {
												$link       = el_site_url("uservalidate/activate/{$guid}/{$activation}");
												$link       = el_call_hook('user', 'validation:email:url', $this, $link);
												$sitename   = el_site_settings('site_name');
												$activation = el_print('el:add:user:mail:body', array(
														$sitename,
														$link,
														el_site_url()
												));
												$subject    = el_print('el:add:user:mail:subject', array(
														$this->first_name,
														$sitename
												));
										...
										el_trigger_callback('user', 'created', array(
												'guid' => $guid
										));
										return $guid;
								}
		...
		public function getUser() {
				...
				if(empty($entities)) {
						...
						return el_call_hook('user', 'get', false, $metadata);
				}
		...
		public function isPassword() {
				$password_minimum = el_call_hook('user', 'password:minimum:length', false, 6);
		...
		public function Login() {
				...
				if($user && $this->verifyPassword($this->password, $user->salt, $user->password) && $user->activation == NULL) {
						...
						ElSession::assign('EL_USER', $user);
						$this->update_last_login();
						
						$vars         = array();
						$vars['user'] = $user;
						$login        = el_call_hook('user', 'login', $vars, true);
						return $login;
				}
				return false;
		}
		...
		public function getFriendRequests($user = '') {
				...
				$this->statement("SELECT * FROM el_relationships WHERE(
				...
				foreach($from as $fr) {
						...
						if(!isset($from->relation_id)) {
								$uss[] = el_user_by_guid($fr->relation_from);
						}
				}
				if(isset($uss)) {
						return $uss;
		...
		public function isFriend($usera, $user2) {
				...
				return el_relation_exists($usera, $user2, 'friend:request', true);
		}
		...
		public function getFriends($user = '', array $options = array()) {
				...
				$relationships = el_get_relationships(array(
						'to' => $user,
						'type' => 'friend:request',
						'inverse' => true,
				...
				if($relationships) {
						foreach($relationships as $relation) {
								$friends[] = el_user_by_guid($relation->relation_to);
						}
						return $friends;
				}
				return false;
		...
		public function sendRequest($from, $to) {
				...
				if(el_add_relation($from, $to, 'friend:request')) {
						return true;
				}
				return false;
		}
		...
		public function requestExists($from, $user) {
				...
				return el_relation_exists($from, $user, 'friend:request');
		}
		...
		public function deleteFriend($from, $to) {
				$this->statement("DELETE FROM el_relationships WHERE(
		...
		public function searchUsers(array $params = array()) {
				...
				$default = array(
						...
						'page_limit' => el_call_hook('pagination', 'page_limit', false, 10), //call hook for page limit
						'count' => false,
				...
				if(isset($options['entities_pairs']) && is_array($options['entities_pairs'])) {
						foreach($options['entities_pairs'] as $key => $pair) {
								...
								if(!empty($pair['name']) && isset($pair['value']) && !empty($operand)) {
										...
										$params['joins'][] = "JOIN el_entities as e{$key} ON e{$key}.owner_guid=u.guid";
										$params['joins'][] = "JOIN el_entities_metadata as emd{$key} ON e{$key}.guid=emd{$key}.guid";
								}
						}
				...
				if($users) {
						foreach($users as $user) {
								$result[] = el_user_by_guid($user->guid);
						}
						return $result;
				}
				return false;
		...
		public function iconURL() {
				...
				foreach(el_user_image_sizes() as $size => $dimensions){
						...
						$url                   = el_site_url("avatar/{$this->username}/{$size}/{$seo}.jpeg");
						$this->iconURLS->$size = $url;
				}
				return el_call_hook('user', 'icon:urls', $this, $this->iconURLS);
		}
		...
		public function profileURL($extends = '') {
				$this->profileurl = el_site_url("u/{$this->username}") . $extends;
				return el_call_hook('user', 'profile:url', $this, $this->profileurl);
		}
		...
		public function SendResetLogin() {
				...
				$emailreset_url = el_site_url("resetlogin?user={$this->username}&c={$this->value}");
				$emailreset_url = el_call_hook('user', 'reset:login:url', $this, $emailreset_url);
				$sitename       = el_site_settings('site_name');
				
				$emailmessage = el_print('el:reset:password:body', array(
						$this->first_name,
						$emailreset_url,
						$sitename
				));
				$emailsubject = el_print('el:reset:password:subject');
		...
		public function deleteUser() {
				...
				if(!empty($this->guid) && !empty($this->username)) {
						...
						if($this->delete($params)) {
								//delete user files
								$datadir = el_get_userdata("user/{$this->guid}/");
								
								if(is_dir($datadir)) {
										ElFile::DeleteDir($datadir);
								...
								//delete annotations
								$this->ElAnnotation->owner_guid = $this->guid;
								$this->ElAnnotation->deleteAnnotationByOwner($this->guid);
								//trigger callback so other components can be notified when user deleted.
								
								//should delete relationships
								el_delete_user_relations($this);
								
								$vars['entity'] = $this;
								el_trigger_callback('user', 'delete', $vars);
								return true;
						}
				}
				return false;
		...
		public function resendValidationEmail() {
				...
				if(!$this->isUserVALIDATED()) {
						$link       = el_site_url("uservalidate/activate/{$this->guid}/{$this->activation}");
						$link       = el_call_hook('user', 'validation:email:url', $this, $link);
						$sitename   = el_site_settings('site_name');
						$activation = el_print('el:add:user:mail:body', array(
								$sitename,
		...
		public function getProfilePhoto() {
				...
				if(!empty($this->guid) && !isset($this->icon_guid)) {
						...
						if(isset($entity[0])){
								...
								$user			= el_user_by_guid($this->guid);
		...
		public function getProfileCover() {
				...
				if(!empty($this->guid) && isset($this->cover_guid)){
					  	return el_get_file($this->cover_guid);
				}			
				if(!empty($this->guid)) {
						$this->owner_guid = $this->guid;
						$this->type       = 'user';
		...
		public static function Logout() {
				unset($_SESSION['EL_USER']);
				@session_destroy();
				$_SESSION['EL_USER'] = false;
		}
		/**
		...
		public function getGenders() {
				...
				$params['from']     = "el_entities as e";
				$params['joins']    = array(
						"JOIN el_entities_metadata AS emd ON e.guid = emd.guid"
				);
				$params["wheres"]   = array(
		...
		public function genderTypes() {
				...
				return el_call_hook('user', 'gender:types', $this, $gender);
		}
		/**
		 * Get total users per month for each year
		 *
		...
		public function onlineByGender($gender = 'male', $count = false, $intervals = 100) {
				...
				$params['joins'][] = "JOIN el_entities as e ON e.owner_guid=u.guid";
				$params['joins'][] = "JOIN el_entities_metadata as emd ON emd.guid=e.guid";
				$params['wheres']  = array(
		...
		public function save() {
				...
				if(parent::save()) {
						...
						if(el_loggedin_user()->guid == $this->guid) {
								$_SESSION['EL_USER'] = el_user_by_guid($this->guid);
						}
						return true;
				}
				return false;
		...
		public function canModerate() {
				$allowed = false;
				if(isset($this->guid) && $this instanceof ElUser) {
						if(($this->type == 'normal' && $this->can_moderate == 'yes') || $this->type == 'admin') {
								$allowed = true;
						}
				}
				return el_call_hook('user', 'can:moderate', $this, $allowed);
		}
		/**
		 * isAdmin
		...
		public static function setLogin($userid) {
				if(!empty($userid)) {
						$user = false;
						if(is_int($userid)) {
								$user = el_user_by_guid($userid);
						}
						if(filter_var($userid, FILTER_VALIDATE_EMAIL)) {
								$user = el_user_by_email($userid);
						}
						if(is_string($userid)) {
								$user = el_user_by_username($userid);
						}
						if($user) {
								ElSession::assign('EL_USER', $user);
								return true;
