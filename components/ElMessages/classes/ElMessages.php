 ...
 */
class elMessages extends elEntities {
		/**
		...
		public function send($from, $to, $message) {
				...
				if($this->insert($params)) {
						...
						el_trigger_callback('message', 'created', $params);
		...
		public function recentChat($to, $count = false) {
				...
				SELECT MAX(m2.id) as id FROM el_messages as m2
							
				INNER JOIN el_entities AS e0 ON e0.owner_guid = m2.id
				INNER JOIN el_entities_metadata AS emd0 ON e0.guid = emd0.guid
       
				INNER JOIN el_entities AS e1 ON e1.owner_guid = m2.id
				INNER JOIN el_entities_metadata AS emd1 ON e1.guid = emd1.guid
		...
		public function searchMessages(array $params = array()) {
				...
				$default      = array(
						...
						'offset' => 1,
						'page_limit' => el_call_hook('pagination', 'messages:list:limit', false, 10), //call hook for page limit
						'count' => false
				);
				...
				if(isset($options['entities_pairs']) && is_array($options['entities_pairs'])) {
						foreach($options['entities_pairs'] as $key => $pair) {
								...
								if(!empty($pair['name']) && isset($pair['value']) && !empty($operand)) {
										...
										$params['joins'][] = "INNER JOIN el_entities as e{$key} ON e{$key}.owner_guid=m.id";
