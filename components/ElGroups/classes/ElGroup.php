 ...
 */
class elGroup extends elObject {
		...
		public function createGroup($params) {
				...
				$this->type       = 'user';
				$this->subtype    = 'elgroup';
				if($params['privacy'] == el_PRIVATE || $params['privacy'] == el_PUBLIC) {
						$this->data->membership = $params['privacy'];
				}
				if($guid = $this->addObject()) {
						el_add_relation($params['owner_guid'], $this->getGuid(), 'group:join');
						el_add_relation($this->getGuid(), $params['owner_guid'], 'group:join:approve');
						
						el_trigger_callback('group', 'add', array(
								'group_guid' => $guid,											   
						));
						return true;
				}
		...
		public function getMembersRequests() {
				...
				foreach($from as $fr) {
						if(!$this->isMember($group, $fr->relation_from)) {
								$users[] = el_user_by_guid($fr->relation_from);
						}
				}
				if(isset($users)) {
						return $users;
		...
		public function getMembers($count = false) {
				...
				$members = el_get_relationships(array(
						'from' => $this->guid,
						'type' => 'group:join:approve',
						'count' => $count
				));
				...
				if($members) {
					foreach($members as $member) {
							$users[] = el_user_by_guid($member->relation_to);
					}
				}
				if(isset($users)) {
						return $users;
		...
		public function sendRequest($from, $group) {
				...
				if(!$this->requestExists($from, $group)) {
						if(el_add_relation($from, $group, 'group:join')) {
								...
								if($this->elDatabase->insert($params)){
										el_trigger_callback('group', 'send:request', array(
											'user_guid' => $from,						
											'group_guid' => $group,
										));
										return true;
		...
		public function approveRequest($from, $group) {
				if($this->requestExists($from, $group)) {
						if(el_add_relation($group, $from, 'group:join:approve')){
								el_trigger_callback('group', 'approve:request', array(
									'user_guid' => $from,						
									'group_guid' => $group,
								));											
								return true;
		...
		public function deleteMember($from, $group) {
				...
				if($this->execute()){
						el_trigger_callback('group', 'delete:member', array(
								'user_guid' => $from,						
								'group_guid' => $group,
						));					
						return true;
		...
		public function searchGroups($q, array $args = array()) {
				$params['from']    = 'el_object';
				$params['type']    = 'user';
				$params['subtype'] = 'elgroup';
				$params['wheres']  = array(
				...
				foreach($search as $group) {
						$groupentity[] = el_get_group_by_guid($group->guid);
				}
				$data = $groupentity;
				return $data;
		}
		...
		public function UploadCover() {
				...
				$this->elFile->owner_guid = $this->guid;
				$this->elFile->type       = 'object';
				$this->elFile->subtype    = 'cover';
				$this->elFile->setFile('coverphoto');
				...
				$this->elFile->setPath('cover/');
				$files = clone $this->elFile;
				if($this->elFile->addFile()) {
						...
						$this->ResetCoverPostition($this->elFile->owner_guid);
						return true;
				}
		}
		...
		public function coverURL() {
				...
				$this->coverurl    = el_site_url("groups/cover/{$this->latestcover->guid}/{$file}");
				return el_call_hook('group', 'cover:url', $this, $this->coverurl);
		}
		...
		public function repositionCOVER($guid, $top, $left) {
				$user = el_get_group_by_guid($guid);
		...
		public function coverParameters($guid) {
				$parameters = el_get_group_by_guid($guid)->cover;
		...
		public function getMyGroups($user) {
				self::initAttributes();
				if(empty($user->guid) || !$user instanceof elUser) {
						return false;
				}
				$params           = array();
				...
				if($myGroups = $this->elDatabase->select($params, true)) {
						$myGroups = $this->elDatabase->select($params, true);
