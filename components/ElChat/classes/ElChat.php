 ...
 */
class elChat extends elMessages {
		/**
		...
		public static function setUserChatSession($user) {
				if(!isset($_SESSION['el_chat_users'])) {
						$_SESSION['el_chat_users'] = array();
				}
				if(!in_array($user->guid, $_SESSION['el_chat_users']) && $user) {
						$_SESSION['el_chat_users'][] = $user->guid;
				}
		}
		...
		public static function GetActiveSessions() {
				if(isset($_SESSION['el_chat_users'])) {
						return array_unique($_SESSION['el_chat_users']);
				}
				return false;
		}
		...
		public static function removeChatTab($tab) {
				if(isset($_SESSION['el_chat_users']) && in_array($tab, $_SESSION['el_chat_users'])) {
						$tab = array_search($tab, $_SESSION['el_chat_users']);
						unset($_SESSION['el_chat_users'][$tab]);
						return true;
				}
				return false;
		}
		...
		public static function AllNew() {
				$friends = el_loggedin_user()->getFriends();
				if(!$friends) {
						return false;
				}
				foreach($friends as $friend) {
						$status = 0;
						if(($friend instanceof elUser) && $friend->isOnline(10)) {
		...
		public function getOnlineFriends($user, $intervals = 100) {
				...
				if($friendsl){
					foreach($friendsl as $fl){
						$friends[] = el_call_hook('user', 'get', false, $fl);		
					}
				}
				return $friends;
		}
		 ...
		 * 
		 * @note this copied from elMessages and edited offest beacuse of #1832
		 * 
