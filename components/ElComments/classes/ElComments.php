 ...
 */
class elComments extends elAnnotation {
		/**
		...
		public function PostComment($subject_id, $guid, $comment, $type = 'post') {
				...
				if($type == 'post') {
						$postc = el_get_object($subject_id);
						if(!$postc) {
								return false;
						}
				}
				...
				if($type == 'entity') {
						$entityc = el_get_entity($subject_id);
						if(!$entityc) {
								return false;
						}
				}
				...
				if($cancomment && $this->addAnnotation()) {
						if(isset($this->comment_image)) {
								...
								$file_path            = rtrim(el_validate_filepath($file), '/');
						...
						el_trigger_callback('comment', 'created', $params);
		...
		public function commentsDeleteAll($subject, $type = 'post') {
				...
				//[B]No callback triggered for elComments::commentsDeleteAll
		...
		public function deleteComment($comment) {
				...
				el_trigger_callback('comment', 'before:delete', $params);
				if($this->deleteAnnotation($comment)) {
						el_trigger_callback('comment', 'delete', $params);
						return true;
				}
				return false;
		}
