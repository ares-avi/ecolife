 ...
 */
class elLikes extends elDatabase {
		/**
		 * Like item
		 *
		...
		public function isLiked($subject_id, $guid, $type = 'post') {
				$this->statement("SELECT * FROM el_likes WHERE (
		...
		public function UnLike($subject_id, $guid, $type = 'post') {
				...
				el_trigger_callback('like', 'before:deleted', $vars);
				if($this->isLiked($subject_id, $guid, $type)) {
						$this->statement("DELETE FROM el_likes WHERE(
	                         subject_id='{$subject_id}' AND guid='{$guid}' AND type='{$type}');");
						if($this->execute()) {
								el_trigger_callback('like', 'deleted', $vars);
								return true;
						}
				}
				return false;
		...
		public function deleteLikes($subject_id, $type = 'post') {
				...
				if($likes){
					$this->statement("DELETE FROM el_likes WHERE(subject_id='{$subject_id}' AND type='{$type}');");
					if($this->execute()) {
							$vars               = array();
							$vars['subject_id'] = $subject_id;
							$vars['type']       = $type;
		...
		public function deleteLikesByOwnerGuid($owner_guid) {
				...
				$this->statement("DELETE FROM el_likes WHERE(guid='{$owner_guid}');");
				if($this->execute()) {
						return true;
				}
				return false;
