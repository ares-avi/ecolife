...
if($fileguid = $file->addFile()) {
		...
		if(isset($resize->{0}->value)) {
				$guid      = $user->guid;
				$datadir   = el_get_userdata("user/{$guid}/{$resize->{0}->value}");
				$file_name = str_replace('profile/photo/', '', $resize->{0}->value);
				
				//create sub photos
				$sizes = el_user_image_sizes();
				foreach($sizes as $size => $params) {
						$params  = explode('x', $params);
						$width   = $params[1];
						$height  = $params[0];
						$resized = el_resize_image($datadir, $width, $height, true);
						file_put_contents(el_get_userdata("user/{$guid}/profile/photo/{$size}_{$file_name}"), $resized);
				}
