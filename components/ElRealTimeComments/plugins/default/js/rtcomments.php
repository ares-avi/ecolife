...
$(document).ready(function() {
	...
	$('body').on('focus', '.comment-box', function() {
		...
		var $status = {
			url: el.site_url + 'action/rtcomments/setstatus?type=' + $btype + '&guid=' + $guid,
			action: true,
			callback: function() {}
		};
...
el.commentTyping = function($guid, $type) {
	//el <5.3 we don't know eather the comment list is for entities comments or the posts comments
	...
	if ($firstlist.length) {
		...
		if ($firstclass.indexOf('el-comments-list') >= 0) {
			if (!$firstlist.hasClass("el-comments-list-" + $type.charAt(0) + "" + $guid)) {
				$firstlist.addClass("el-comments-list-" + $type.charAt(0) + "" + $guid);
			}
		}
	}
	var $cguid = $guid;
	...
	var $status = {
		url: el.site_url + 'action/rtcomments/status?guid=' + $cguid + '&type=' + $ctype,
		action: true,
			...
			if (callback['lists'] != '') {
				for(i=0;i < callback['lists'].length;i++){
					...
					if($id && $('#comments-item-'+$id).length == 0){
						$(".el-comments-list-" + $type.charAt(0) + "" + $guid).append(callback['lists'][i]).fadeIn();	
					}
				}
			}
		}
	...
	setInterval(function() {
		if ($('.ctyping-' + $ctype + '-' + $cguid).isInViewComments()) {
			if ($(".el-comments-list-" + $type.charAt(0) + "" + $guid).find('.comments-item').length) {
				var $ids = new Array();
				$($(".el-comments-list-" + $type.charAt(0) + "" + $guid).find('.comments-item')).each(function() {
