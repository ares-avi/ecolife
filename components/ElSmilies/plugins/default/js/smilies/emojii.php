el.RegisterStartupFunction(function() {
	...
	$(document).ready(function() {
		...
		$.each(EmojiiArray, function(key, data) {
			firstele = data[0];
			$('.emojii-container').find('.nav-tabs').append("<li class='el-emojii-tab' data-type='" + key + "'><a class='emojii' href='javascript:void(0);'>&#x" + firstele + ";</a></li>");
		...
		$('body').on('click', '.el-emojii-tab', function(e) {
			e.preventDefault();
		...
		// 1. comment
		// the button/icon is added now using el_extend_view('comments/attachment/buttons', 'smilies/comment/button'); to avoid the issue for dynamically loaded posts/comments $arsalanshah

		// 2. wall post
		// inserted as registered menu item in el_com

		// 3. messages page:
		if ($('.message-form-form').length) {
			$('<div class="el-message-attach-photo"><i class="fa fa-smile-o"></i></div>').prependTo('.message-form-form .controls');
		}

		// 4. chatbox
		// inserted by elChat component
		...
		// 1. comment
		$('body').on('click', '.el-comment-attach-photo .fa-smile-o', function(e) {
			$parent = $(this).parent().parent().parent();
			el.OpenEmojiBox('#' + $parent.find('.comment-box').attr('id'));
		});

		// 2. wall post
		$('body').on('click', '.el-wall-container-control-menu-emojii-selector', function(e) {
			el.OpenEmojiBox('.el-wall-container-data textarea');
		});

		// 3. message
		$('body').on('click', '.el-message-attach-photo .fa-smile-o', function(e) {
			el.OpenEmojiBox('.message-form-form textarea');
		});
		...
		el.OpenEmojiBox = function(anchor) {
		...
		$('body').on('click', '#master-moji .emojii-list li', function(e) {
			...
			// be the same all over el in the future
