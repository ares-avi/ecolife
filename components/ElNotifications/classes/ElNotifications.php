 ...
 */
class elNotifications extends elDatabase {
		/**
		...
		public function add($type, $poster_guid, $subject_guid, $item_guid = NULL, $notification_owner = '') {
				if(!empty($type) && !empty($subject_guid) && !empty($poster_guid)) {
						...
						$this->notification = el_call_hook('notification:add', $type, $vars, false);
						...
						if($this->insert($params)) {
								...
								if(el_call_hook('notification:participants', $this->notification['type'], NULL, true)) {
										//notify participates
												...
												foreach($paricipates as $partcipate) {
														...
														if($partcipate !== $poster_guid) {
																if($this->insert($params)) {
																		...
																		el_trigger_callback('notification', 'add:participates', $callback);
																}
														}
												}
										}
								}
								el_trigger_callback('notification', 'add', $callback);
								return true;
						}
				}
				return false;
		...
		public function get($guid_two = '', $unread = false, $limit = false, $count = false) {
				...
				if($get) {
						foreach($get as $notif) {
								if(el_is_hook('notification:view', $notif->type)) {
										$messages[] = el_call_hook('notification:view', $notif->type, $notif);
								}
						}
						return $messages;
				}
		...
		public function setViewed($guid) {
				$this->statement("UPDATE el_notifications SET viewed='' WHERE(guid='{$guid}');");
				if($this->execute()) {
						return true;
				}
				return false;
		...
		public function deleteUserNotifications($user) {
				if($user) {
						$this->statement("DELETE FROM el_notifications WHERE(
		...
		public function searchNotifications(array $params = array()) {
				...
				$params             = array();
				$params['from']     = 'el_notifications as n';
				$params['params']   = array(
						'n.*'
				);
				$params['wheres']   = array(
		...
		public function toTemplate() {
				...
				if(el_is_hook('notification:view', $this->type)) {
						return el_call_hook('notification:view', $this->type, $this);
				}
				return false;
		}
}
