...
/**
 * Library modifed for el, Original code is from Cash Costello.
 * Embed Video Library
 ...
 */
function el_embed_create_embed_object($url, $guid, $videowidth=0) {
	...
	if (strpos($url, 'youtube.com') != false) {
		...
		$url = str_replace('m.youtube.com', 'www.youtube.com', $url);
 		return el_embed_youtube_handler($url, $guid, $videowidth);
	} else if(strpos($url, 'm.youtube.com') != false) {
		return el_embed_youtube_handler($url, $guid, $videowidth);		
	} else if (strpos($url, 'youtu.be') != false) {
		return el_embed_youtube_shortener_parse_url($url, $guid, $videowidth);
	} else if (strpos($url, 'vimeo.com') != false) {
		return el_embed_vimeo_handler($url, $guid, $videowidth);
	} else if (strpos($url, 'metacafe.com') != false) {
		return el_embed_metacafe_handler($url, $guid, $videowidth);
	} else if (strpos($url, 'dailymotion.com') != false) {
		return el_embed_dm_handler($url, $guid, $videowidth);
	} else if (strpos($url, 'hulu.com') != false) {
		return el_embed_hulu_handler($url, $guid, $videowidth);
	} else {
		return false;
	}
}
 ...
 */
function el_embed_add_css($guid, $width, $height) {
	...
	return el_call_hook('embed', 'video:css', $vars, $videocss);
}
 ...
 */
function el_embed_add_object($type, $url, $guid, $width, $height) {
	$videodiv = "<span id=\"elembed{$guid}\" class=\"el_embed_video embed-responsive embed-responsive-16by9\">";

	// could move these into an array and use sprintf
	switch ($type) {
		case 'youtube':
			//youtube https in elembed.lib.php #519
	...
	// hmm no need for div post-text without ending tag , removed it from here and removed ending tag from el_embed_add_css() 
 ...
 */
function el_embed_calc_size(&$width, &$height, $aspect_ratio, $toolbar_height) {
 ...
 */
function el_embed_youtube_handler($url, $guid, $videowidth) {
	// this extracts the core part of the url needed for embeding
	$videourl = el_embed_youtube_parse_url($url);
	if (!isset($videourl)) {
		return false;
	}

	el_embed_calc_size($videowidth, $videoheight, 425/320, 24);

	$embed_object = el_embed_add_css($guid, $videowidth, $videoheight);

	$embed_object .= el_embed_add_object('youtube', $videourl, $guid, $videowidth, $videoheight);
 ...
 */
function el_embed_youtube_parse_url($url) {
 ...
 */
function el_embed_youtube_shortener_parse_url($url) {
 ...
 * @return string css style, video div, and flash <object>
 */
function el_embed_vimeo_handler($url, $guid, $videowidth) {
	// this extracts the core part of the url needed for embeding
	$videourl = el_embed_vimeo_parse_url($url);
	if (!isset($videourl)) {
		return false;
	}

	// aspect ratio changes based on video - need to investigate
	el_embed_calc_size($videowidth, $videoheight, 400/300, 0);

	$embed_object = el_embed_add_css($guid, $videowidth, $videoheight);

	$embed_object .= el_embed_add_object('vimeo', $videourl, $guid, $videowidth, $videoheight);
 ...
 * @return string hash
 */
function el_embed_vimeo_parse_url($url) {
	// separate parsing embed url
	if (strpos($url, 'object') != false) {
		return el_embed_vimeo_parse_embed($url);
	}
 ...
 * @return string hash
 */
function el_embed_vimeo_parse_embed($url) {
 ...
 * @return string css style, video div, and flash <object>
 */
function el_embed_metacafe_handler($url, $guid, $videowidth) {
	// this extracts the core part of the url needed for embeding
	$videourl = el_embed_metacafe_parse_url($url);
	if (!isset($videourl)) {
		return false;
	}

	el_embed_calc_size($videowidth, $videoheight, 400/295, 40);

	$embed_object = el_embed_add_css($guid, $videowidth, $videoheight);

	$embed_object .= el_embed_add_object('metacafe', $videourl, $guid, $videowidth, $videoheight);
 ...
 * @return string hash
 */
function el_embed_metacafe_parse_url($url) {
	// separate parsing embed url
	if (strpos($url, 'embed') != false) {
		return el_embed_metacafe_parse_embed($url);
	}
 ...
 * @return string hash
 */
function el_embed_metacafe_parse_embed($url) {
 ...
 * @return string css style, video div, and flash <object>
 */
function el_embed_dm_handler($url, $guid, $videowidth) {
	// this extracts the core part of the url needed for embeding
	$videourl = el_embed_dm_parse_url($url);
	if (!isset($videourl)) {
		return false;
	}

	el_embed_calc_size($videowidth, $videoheight, 420/300, 35);

	$embed_object = el_embed_add_css($guid, $videowidth, $videoheight);

	$embed_object .= el_embed_add_object('dm', $videourl, $guid, $videowidth, $videoheight);
 ...
 * @return string hash
 */
function el_embed_dm_parse_url($url) {
	if (strpos($url, 'embed') != false) {
		return false;
	}
 ...
 * @return string css style, video div, and flash <object>
 */
function el_embed_hulu_handler($url, $guid, $videowidth) {
	// this extracts the core part of the url needed for embeding
	$videourl = el_embed_hulu_parse_url($url);
	if (is_numeric($videourl)) {
		return false;
	}

	el_embed_calc_size($videowidth, $videoheight, 512/296, 0);

	$embed_object = el_embed_add_css($guid, $videowidth, $videoheight);

	$embed_object .= el_embed_add_object('hulu', $videourl, $guid, $videowidth, $videoheight);
 ...
 * @return string hash
 */
function el_embed_hulu_parse_url($url) {
	// separate parsing embed url
	if (strpos($url, 'embed') === false) {
		return 1;
	}
