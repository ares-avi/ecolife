...
class ElInstallation {
		
		public static function url() {
				return str_replace('installation/', '', el_installation_paths()->url);
		}
		...
		public static function is_mod_rewrite() {
				...
				$file    = el_url();
				$rewrite = el_installation_simple_curl($file . 'rewrite.php');
				if($rewrite == 1) {
						return true;
				}
				return false;
		...
		public function INSTALL() {
				if(stripos($this->datadir, $this->elInstallationDir()) === 0) {
						$this->error_mesg = el_installation_print('data:directory:outside');
						return false;
				}
				if(!is_dir($this->datadir) && !is_writable($this->datadir)) {
						$this->error_mesg = el_installation_print('data:directory:invalid');
						return false;
				}			
				...
				if($script = file_get_contents(el_installation_paths()->root . 'sql/opensource-socialnetwork.sql')) {
						...
						$script         = str_replace("<<secret>>", substr(md5('el' . bin2hex(random_bytes(6))), 3, 8), $script);
						$errors         = array();
						$script         = preg_replace('/\-\-.*\n/', '', $script);
		 ...
		 * @return string
		 */
		public static function elInstallationDir() {
		...
		function configurations_db() {
				...
				$this->path   = str_replace('installation/', '', el_installation_paths()->root);
				$templateFile = $this->path . "configurations/el.config.db.example.php";
				$template     = file_get_contents($templateFile);
				if(!$template) {
						throw new Exception(el_installation_print('all:files:required'));
				}
				...
				$settingsFilename = $this->path . "configurations/el.config.db.php";
		...
		function configurations_site() {
				...
				$this->path   = str_replace('installation/', '', el_installation_paths()->root);
				$templateFile = $this->path . "configurations/el.config.site.example.php";
				$template     = file_get_contents($templateFile);
				if(!$template) {
						throw new Exception(el_installation_print('all:files:required'));
				}
				...
				$settingsFilename = $this->path . "configurations/el.config.site.php";
				$result           = file_put_contents($settingsFilename, $template);
				if(!$result) {
						return false;
				}
