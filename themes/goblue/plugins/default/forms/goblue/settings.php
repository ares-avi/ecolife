 <fieldset class="titleform">
 	<div class="alert alert-warning">
    	<?php echo el_print('theme:goblue:browercache');?>
    </div>	
 	<div>	
    	<label><?php echo el_print('theme:goblue:logo:site');?> (450x90 - 500 KB PNG) </label>
        <input type="file" name="logo_site" />
        <div class="logo-container-goblue">
            	<?php if(el_site_settings('cache') == true){?>
            	<img src="<?php echo el_theme_url();?>images/logo.png" />
                <?php } else { ?>
            	<img src="<?php echo el_theme_url();?>images/logo.png?v=<?php echo time();?>" />                
                <?php } ?>
        </div>
    </div>
  	<div>	
    	<label><?php echo el_print('theme:goblue:logo:admin');?> (180x45 - 500 KB JPG)</label>
