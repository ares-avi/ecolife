...
$args_url = elPagination::constructUrlArgs();
...
if (count($pages) !== 1) {
	...
	//get key by offset
	$key = el_recursive_array_search($params['offset'], $serialized_pages);
	...
	<div class="container-table container-table-pagination">
		<div class="center-row">
				<ul class="pagination el-pagination">';
  	//disaply first if first page is exist
	if(isset($first) && !empty($first)){
        $url = "{$args['href']}?{$args['offset_name']}={$first}{$args_url}";
     	echo "<li><a href='{$url}' class='el-pagination-page'>".el_print('el:pagination:first')."</a></li>";	
	} 
   foreach ($pages as $page) {
	...
	if(isset($last) && !empty($last)){
        $url = "{$args['href']}?{$args['offset_name']}={$last}{$args_url}";
     	echo "<li><a href='{$url}'>".el_print('el:pagination:last')."</a></li>";	
	}
echo '</ul></div></div>';
}
