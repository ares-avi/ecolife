...
$(document).ready(function() {
	...
	$(document).on('click', '#sidebar-toggle', function() {
		...
		if ($toggle == 0) {
			...
			if($(document).innerWidth() >= 1300 && $('.el-page-loading-annimation').is(':visible')){
				$('.sidebar').addClass('sidebar-open-no-annimation');	
				$('.el-page-container').addClass('sidebar-open-page-container-no-annimation');
			} else {
				$('.sidebar').addClass('sidebar-open');
				$('.el-page-container').addClass('sidebar-open-page-container');
			}			
			$('.topbar .right-side').addClass('right-side-space');
			$('.topbar .right-side').addClass('sidebar-hide-contents-xs');
			$('.el-inner-page').addClass('sidebar-hide-contents-xs');
		}
		if ($toggle == 1) {
			...
			$('.el-page-container').removeClass('sidebar-open-page-container');
			$('.el-page-container').removeClass('sidebar-open-page-container-no-annimation');
			$('.topbar .right-side').removeClass('right-side-space');
			$('.topbar .right-side').removeClass('sidebar-hide-contents-xs');
			$('.el-inner-page').removeClass('sidebar-hide-contents-xs');

			$('.topbar .right-side').addClass('right-side-nospace');
			$('.sidebar').addClass('sidebar-close');
			$('.el-page-container').addClass('sidebar-close-page-container');

		}
		var document_height = $(document).height();
		$(".sidebar").height(document_height);
	});
	var $chatsidebar = $('.el-chat-windows-long .inner');
	...
	$(document).scroll(function() {
		...
		if($chatsidebar.length){
			if ($(document).scrollTop() >= 50) {
				$chatsidebar.addClass('elchat-scroll-top');
				$chatsidebar.css('height', $(window).height());
			} else if ($(document).scrollTop() == 0) {
				$chatsidebar.removeClass('elchat-scroll-top');
				$chatsidebar.css('height', $(window).height() - 45);
			}
		}
	});
...
$(document).ready(function() {
	   $(".el-page-loading-annimation").fadeOut("slow");
});
$(window).on('load resize', function () {
	if (document.querySelector("#draggable")) {
		if (($('.el-group-cover').height() + $('.profile-cover').height()) < 481) {
			// we're on mobile and have a group cover image
			const desktop_cover_width  = 1040;
			const desktop_cover_height = 200;
			var mobile_cover_height  = $('.el-group-cover').height() + $('.profile-cover').height();
			...
			var current_cover_width = $('.el-group-cover').width() + $('.profile-cover').width();
